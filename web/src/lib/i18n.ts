...
export const SUPPORTED_LANGUAGES_PAIRS = [
  ...
  // Russian at the top (primary language for this deployment), then English, then alphabetically
...
void i18next
  ...
  .init({
    resources,
    // Default language is Russian; users can still switch via the
    // language selector, and browser/localStorage detection (below)
    // still takes priority if the user has already chosen a language.
    lng: undefined, // let the detector decide first
    fallbackLng: 'ru',
    defaultNS: 'common',
    interpolation: {
