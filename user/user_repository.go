...
func NewUserRepository(dbDir string, databaseUrl string, maxSavedLoc int, maxSavedPic int) UserRepository {
	db := NewFMDDB(dbDir, databaseUrl)
...
func (u *UserRepository) RequestAccess(username string, innerPwHash string, sessionDurationSeconds uint64, remoteIp string, totpCodes ...string) (*FMDUser, *AccessToken, error) {
	...
	if actual == expected {
		if user.TotpEnabled {
			...
			if !u.ValidateTotpCode(user, totpCode) {
				u.ACC.IncrementLock(username)
				u.LogAuditEvent(user, AuditLoginFailed, remoteIp)
				log.Warn().
					Str("user", user.Username).
					Str("remoteIp", remoteIp).
					Msg("failed login attempt (bad 2FA code)")
				return nil, nil, ErrTotpCodeInvalid
			}
		}

		u.ACC.ResetLock(username)
		u.LogAuditEvent(user, AuditLoginSuccess, remoteIp)
